...
bool getData(TF_Luna_Lidar *tf_luna, int16_t *dist, int16_t *flux, int16_t *temp)
{
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // Step 1 - Use the `HAL_I2C_MASTER_Receive` function `readRegluna` to fill the six byte
    ...
    {
      if( !readRegluna(tf_luna, reg)) return false;
...
bool Get_Time(TF_Luna_Lidar *tf_luna, uint16_t *tim)
{
    ...
    if( !readRegluna(tf_luna, TFL_TICK_LO)) return false;
        else p_tim[ 0] = regReply;  // Read into `tim` array
    if( !readRegluna(tf_luna, TFL_TICK_HI)) return false;
...
bool Get_Prod_Code(TF_Luna_Lidar *tf_luna, uint8_t * p_cod)
{
   for (uint8_t i = 0; i < 14; ++i)
    {
      if( !readRegluna(tf_luna, (0x10 + i))) return false;
...
bool Get_Firmware_Version(TF_Luna_Lidar *tf_luna, uint8_t * p_ver)
{
    ...
    {
      if( !readRegluna(tf_luna, (0x0A + i))) return false;
...
{
    return(writeRegluna(tf_luna, TFL_SAVE_SETTINGS, 1));
}

//  = = = =   SOFT (SYSTEM) RESET   = = = =
bool Soft_Reset( TF_Luna_Lidar *tf_luna)
{
    return(writeRegluna(tf_luna, TFL_SOFT_RESET, 2));
}
...
{
    return(writeRegluna(tf_luna, TFL_SET_I2C_ADDR, adrNew));
}

//  = = = = =   SET ENABLE   = = = = =
bool Set_Enable( TF_Luna_Lidar *tf_luna)
{
    return(writeRegluna(tf_luna, TFL_DISABLE, 1));
}

//  = = = = =   SET DISABLE   = = = = =
bool Set_Disable( TF_Luna_Lidar *tf_luna)
{
    return(writeRegluna(tf_luna, TFL_DISABLE, 0));
}
...
bool Set_Frame_Rate(TF_Luna_Lidar *tf_luna, uint16_t *frm)
{
    ...
    // ... then address the pointer as an array.
    if( !writeRegluna(tf_luna, (TFL_FPS_LO), p_frm[0])) return false;
    if( !writeRegluna(tf_luna, (TFL_FPS_HI), p_frm[1])) return false;
    return true;
}
...
{
    uint8_t * p_frm = (uint8_t *) *&frm;
    if( !readRegluna(tf_luna, TFL_FPS_LO)) return false;
        else p_frm[ 0] = regReply;  // Read into `frm` array
    if( !readRegluna(tf_luna, TFL_FPS_HI)) return false;
...
{
    return(writeRegluna(tf_luna, TFL_HARD_RESET, 1));
}
...
{
    return(writeRegluna(tf_luna, TFL_SET_TRIG_MODE, 0));
}
...
{
    return(writeRegluna(tf_luna, TFL_SET_TRIG_MODE, 1));
}
...
{
    return(writeRegluna(tf_luna, TFL_TRIGGER, 1));
}
//
// = = = = = = = = = = = = = = = = = = = = = = = =
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool readRegluna(TF_Luna_Lidar *tf_luna, uint8_t nmbr)
{
...
bool writeRegluna(TF_Luna_Lidar *tf_luna, uint8_t nmbr, uint8_t data)
{
